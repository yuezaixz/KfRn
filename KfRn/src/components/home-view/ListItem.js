...
class ListItem extends Component {
    handleConnect = ()=>{
        ...
        if (this.props.home_data.leftDevice.uuid == this.props.data.uuid) {
            return;
        }
    ...
    render() {
        return (
            <View style={[styles.container, {borderBottomWidth: this.props.isLast?0:1}]}>
                ...
                <Text style={styles.timer}>{this.props.home_data.isConnecting && this.props.home_data.connecting_uuid==this.props.data.uuid?'连接中':(this.props.data.uuid==this.props.home_data.leftDevice.uuid?'已连接':this.props.data.rssi)}</Text>
            </View>
        );
    }
}
